...
namespace HotelV4.aclass
{
    public class RoomDAO
    {
        ...
        {
            return cdb.Instance.ExecuteQuery("USP_LoadFullRoom");
        }
        ...
        internal bool InsertRoom(string roomName, int idRoomType, int idStatusRoom)
        {
            ...
            return cdb.Instance.ExecuteNoneQuery(query, new object[] { roomName, idRoomType, idStatusRoom }) > 0;
        }
        internal bool UpdateCustomer(Room roomNow)
        {
            string query = "USP_UpdateRoom  @id , @nameRoom , @idRoomType , @idStatusRoom";
            return cdb.Instance.ExecuteNoneQuery(query, new object[] { roomNow.Id, roomNow.Name, roomNow.IdRoomType, roomNow.IdStatusRoom }) > 0;
        }
        ...
        public List<Room> LoadEmptyRoom(int idRoomType)
        {
            ...
            DataTable data = cdb.Instance.ExecuteQuery(query, new object[] { idRoomType });
        ...
        public List<Room> LoadListFullRoom()
        {
            ...
            List<Room> rooms = new List<Room>();
            DataTable data = cdb.Instance.ExecuteQuery(query, new object[] { DateTime.Now.Date });
            foreach (DataRow item in data.Rows)
            {
                Room room = new Room(item);
                rooms.Add(room);
        ...
        public int GetPeoples(int idBill)
        {
            ...
            return (int)cdb.Instance.ExecuteScalar(query, new object[] { idBill }) + 1;
        }
        ...
        public bool UpdateStatusRoom(int idRoom)
        {
            ...
            return cdb.Instance.ExecuteNoneQuery(query, new object[] { idRoom }) > 0;
        }
        #endregion
